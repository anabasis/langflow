---
title: "문제 해결"
source_url: "https://docs.langflow.org/next/troubleshoot"
source_commit: "3ec070e99af5196fdb187546de2c4f29a35ebe46"
source_file: "Support/troubleshooting.mdx"
translation_type: "한국어 요약 번역"
---

# 문제 해결

Langflow 사용 중 **문제 해결** 관련 상태를 확인하고 후속 조치를 수행하기 위한 지원 안내서임.

> 이 문서는 공식 Langflow 문서를 바탕으로 작성한 한국어 요약 매뉴얼임. 버전별 옵션과 전체 예제는 아래 공식 원문 확인 필요.

## 문서 정보

- 공식 문서: [https://docs.langflow.org/next/troubleshoot](https://docs.langflow.org/next/troubleshoot)
- 원본 소스: [Support/troubleshooting.mdx](https://github.com/langflow-ai/langflow/blob/3ec070e99af5196fdb187546de2c4f29a35ebe46/docs/docs/Support/troubleshooting.mdx)
- 기준 커밋: `3ec070e99af5196fdb187546de2c4f29a35ebe46`

## 주요 내용

- Missing components
- No input in the Playground
- Missing key, no key found, or invalid API key
- Langflow installation issues
- Langflow installation freezes at pip dependency resolution
- Linux installation fails to build required package
- Installation failure from `webrtcvad` package
- `get_body_field` error after installing Langflow
- Langflow Desktop installation issues
- Protocol buffers (protoc) required for Intel-based Macs
- Langflow installation failed
- Langflow startup failed
- Failed to install uv or unable to find uv
- Python environment failed

## 사용 절차

1. 공식 문서의 사전 요구 사항과 지원 버전 확인.
2. 원문 순서에 따라 설정값과 연결 관계 구성.
3. 최소 예제로 기능 동작 확인.
4. 로그와 출력값을 기준으로 오류 여부 검증.
5. 운영 적용 전 보안·성능·복구 기준 점검.

## 적용 시 주의 사항

- 명령어, API 경로, JSON 키, 환경 변수명 및 컴포넌트 이름은 원문 표기를 유지할 것.
- 실행 전 설치 버전과 공식 문서의 지원 범위를 확인할 것.

## 원문 세부 내용 확인

설정 필드, 전체 코드 예제, 버전별 제한 사항과 화면 이미지는 [공식 문서](https://docs.langflow.org/next/troubleshoot) 및 [GitHub 원본](https://github.com/langflow-ai/langflow/blob/3ec070e99af5196fdb187546de2c4f29a35ebe46/docs/docs/Support/troubleshooting.mdx)에서 확인 가능.
